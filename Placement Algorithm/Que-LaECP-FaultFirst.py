...
["fault-ensemble", False, True, 3, faultDetectionCPU[5], faultDetectionMem[5], 0, 0, 3, 12752*3, 49, 1, thisTotalCount,faultcomputeDemand[5]],#5
    ["wafer-source", True, False, 0, waferInspectionCPU[0], waferInspectionCPU[0], 2, 552769, 0, 0, 0.05, 1, thisTotalCount,wafercomputeDemand[0]],#6
...
#["rul-preprocess", False, False, 1, rulPredictionCPU[1], rulPredictionMem[1], 1, 63565202, 1, 3509033, 0.02, 5, 29],
    ...
    ["wafer-ensemble", False, True, 3, waferInspectionCPU[5], waferInspectionCPU[5], 0, 0, 2, 874, 52, 1, thisTotalCount,wafercomputeDemand[5]],#11
...
oneGraphAppServices[5].setLatencyRe(faultDetectionLatencyCons)
oneGraphAppServices[11].setLatencyRe(waferInspectionLatencyCons)

oneGraphAppServices[1].setInnode([oneGraphAppServices[0]])
oneGraphAppServices[2].setInnode([oneGraphAppServices[1]])
oneGraphAppServices[3].setInnode([oneGraphAppServices[1]])
oneGraphAppServices[4].setInnode([oneGraphAppServices[1]])
oneGraphAppServices[5].setInnode([oneGraphAppServices[2],oneGraphAppServices[3],oneGraphAppServices[4]])

oneGraphAppServices[7].setInnode([oneGraphAppServices[6]])
oneGraphAppServices[8].setInnode([oneGraphAppServices[6]])
oneGraphAppServices[9].setInnode([oneGraphAppServices[7]])
oneGraphAppServices[10].setInnode([oneGraphAppServices[8]])
oneGraphAppServices[11].setInnode([oneGraphAppServices[9],oneGraphAppServices[10]])
...
oneGraphAppServices[6].setDevice(camera)
...
def threeLatency(uplatency,bandwidth,microSubvolum,totalcount,microComputDemand,deviceCoputeTotal):
    # 分Represents the time inverse of the microserver in the device, the data arrival rate, bandwidth, and data volume
    comm1 = microSubvolum / (bandwidth[0]*1024*1024/8)
    compute1 = totalcount * (microComputDemand / deviceCoputeTotal) * (
            1 / (deviceCoputeTotal - totalcount * microComputDemand))
    total1 = comm1 + compute1
...
print(dataPD)
dataPD.to_csv('./result/infoForeachPlace/queue-faultfirst-'+str(thisTotalCount)+'.csv')
...
sourcelossofilename = './result/allplacement/' + str(thisTotalCount) +"-queue--faultFirst--allPlacement.csv"
features1=["placement"]
