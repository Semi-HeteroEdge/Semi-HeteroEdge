...
#----------------------------------------------01----------------------------------------------------------
totalcount=40
attribute = pd.read_csv(str(totalcount)+'-mix-one-data-1.csv')
label = pd.read_csv(str(totalcount)+'-mix-one-label.csv')
...
def xgboost_factory(argsDict):
    ...
    joblib.dump(xrf, str(totalcount)+'-mix-onexgboost-1.pkl')
    plot_importance(xrf)
    plt.subplots_adjust(left=0.30, wspace=0.25, hspace=0.25,
                        bottom=0.13, top=0.91)
    plt.savefig(str(totalcount)+"-mix-onexgboost-1.png")
