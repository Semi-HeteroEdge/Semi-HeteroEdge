...
attribute = pd.read_csv(str(totalcount)+"-"+Devide+'-three-data.csv')
label = pd.read_csv(str(totalcount)+"-"+Devide+'-three-label.csv')
...
def xgboost_factory(argsDict):
    ...
    joblib.dump(xrf, str(totalcount)+'-only-threexgboost-'+Devide+'.pkl')
    plot_importance(xrf,max_num_features=20)
    plt.subplots_adjust(left=0.30, wspace=0.25, hspace=0.25,
                       bottom=0.13, top=0.91)

    plt.savefig(str(totalcount)+"-only-threexgboost-"+Devide+".png")
    plt.show()
