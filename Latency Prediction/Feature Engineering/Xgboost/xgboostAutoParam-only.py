...
totalcount=100
Devide="edge"
attribute = pd.read_csv(str(totalcount)+'-'+Devide+'-one-data.csv')
label = pd.read_csv(str(totalcount)+'-'+Devide+'-one-label.csv')
...
def xgboost_factory(argsDict):
    ...
    joblib.dump(xrf, str(totalcount)+'-only-onexgboost-'+Devide+'.pkl')
    plot_importance(xrf)
    plt.subplots_adjust(left=0.30, wspace=0.25, hspace=0.25,
                        bottom=0.13, top=0.91)
    plt.savefig(str(totalcount)+"-only-onexgboost-"+Devide+".png")
